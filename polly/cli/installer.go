...
func installSystemD(exeFile string) {
	...
	cmd := exec.Command("systemctl", "enable", "-q", "polly.service")
	...
	fmt.Print("Polly is now installed. Before starting it please check ")
	fmt.Print("http://github.com/emccode/polly for instructions on how to ")
	fmt.Print("configure it.\n\n Once configured the Polly service can be ")
	fmt.Print("started with the command 'sudo systemctl start polly'.\n\n")
}

func uninstallSystemD() {

	// a link created by systemd as docker should "want" polly as a service.
	// the uninstaller will fail
	os.Remove("/etc/systemd/system/docker.service.wants/polly.service")

	cmd := exec.Command("systemctl", "disable", "-q", "polly.service")
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr
	err := cmd.Run()
...
func installUpdateRcd(exeFile string) {
	...
	cmd := exec.Command("update-rc.d", "polly", "defaults")
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr
	err := cmd.Run()
	...
	fmt.Print("http://github.com/emccode/polly for instructions on how to ")
	fmt.Print("configure it.\n\n Once configured the Polly service can be ")
	fmt.Print("started with the command 'sudo /etc/init.d/polly start'.\n\n")
}
...
func uninstallUpdateRcd() {
	...
	cmd := exec.Command("update-rc.d", "polly", "remove")
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr
	err := cmd.Run()
...
func installChkConfig(exeFile string) {
	createInitFile(exeFile)
	cmd := exec.Command("chkconfig", "polly", "on")
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr
	err := cmd.Run()
...
func uninstallChkConfig() {
	cmd := exec.Command("chkconfig", "--del", "polly")
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr
	err := cmd.Run()
