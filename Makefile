...
NV := $$($(GLIDE) novendor)
BASEPKG := github.com/emccode/polly
...
build_:
	...
	@printf "  ...building polly $(V_OS_ARCH)..."; \
		...
		env GOOS=$(_GOOS) GOARCH=$(_GOARCH) go build -o $$FILE $(GOFLAGS) $(LDFLAGS) ./polly; \
		$(PRINT_STATUS); \
		if [ "$$EC" -eq "0" ]; then \
			mkdir -p $(DEPLOY)/$(V_OS_ARCH); \
			mkdir -p $(DEPLOY)/latest; \
			cd $(BINDIR)/$(V_OS_ARCH); \
			TARBALL=polly-$(V_OS_ARCH)-$(V_SEMVER).tar.gz; \
			LATEST=polly-$(V_OS_ARCH).tar.gz; \
			tar -czf $$TARBALL polly; \
...
_install: _deps _fmt
	...
	@printf "  ...installing polly $(V_OS_ARCH)..."; \
		...
		go install $(GOFLAGS) $(LDFLAGS) ./polly/; \
		$(PRINT_STATUS); \
		if [ "$$EC" -eq "0" ]; then \
...
rpm:
	...
	@printf "  ...building rpm $(V_ARCH)..."; \
		...
		mkdir -p $(RPMDIR)/BUILD \
				 ...
				 $(RPMDIR)/SOURCES \
				 $(RPMDIR)/tmp; \
		cp $(BUILDS)/polly.spec $(RPMDIR)/SPECS/polly.spec; \
		cd $(RPMDIR); \
		setarch $(V_ARCH) rpmbuild -ba --quiet \
			-D "rpmbuild $(CWD)/$(RPMDIR)" \
			-D "v_semver $(V_RPM_SEMVER)" \
			-D "v_arch $(V_ARCH)" \
			-D "polly $(CWD)/$(BINDIR)/$(V_OS_ARCH)/polly" \
			SPECS/polly.spec; \
		$(PRINT_STATUS); \
		if [ "$$EC" -eq "0" ]; then \
